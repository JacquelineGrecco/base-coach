...
interface DbSessionWithRelations {
  id: string;
  team_id: string;
...
interface DbEvaluationWithPlayer {
  id: string;
  session_id: string;
  player_id: string;
  valence_id: string;
...
export interface SessionEvaluation {
  ...
  session_id: string;
  player_id: string;
  valence_id: string; // Renamed from valence to valence_id (matches DB schema)
  score: number; // 0-5
  notes?: string;
  created_at: string;
  updated_at: string;
...
export const sessionService = {
  ...
  async getSessions(teamId?: string): Promise<{ sessions: Session[] | null; error: Error | null }> {
    try {
      ...
      const sessions = (data || []).map((session: DbSessionWithRelations) => ({
        ...session,
  ...
  async getSession(sessionId: string): Promise<{ 
    ...
    try {
      ...
      const evaluations = (evaluationsData || []).map((evaluation: DbEvaluationWithPlayer) => ({
        ...evaluation,
  ...
  async getPlayerStats(playerId: string): Promise<{ 
    ...
    try {
      ...
      const statsByValence = (data || []).reduce((acc: Record<string, ValenceStats>, evaluation: DbEvaluation) => {
      ...
      // Calculate averages
      const stats: PlayerStats[] = Object.entries(statsByValence).map(([valence, data]) => ({
        valence_id: valence,
        average: data.total / data.count,
        count: data.count,
        min: Math.min(...data.scores),
        max: Math.max(...data.scores),
        trend: 0, // Calculate trend elsewhere if needed
        scores: data.scores,
      }));
