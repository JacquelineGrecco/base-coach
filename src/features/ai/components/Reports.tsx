...
import { sessionService, SessionEvaluation } from '@/features/match/services/sessionService';
import { teamService, Team } from '@/features/roster/services/teamService';
import jsPDF from 'jspdf';
import { generatePlayerInsights, PlayerInsights } from '@/features/ai/services/geminiService';
import { subscriptionService, SubscriptionInfo, TIER_FEATURES } from '@/features/auth/services/subscriptionService';
import { UpgradePrompt } from '@/components/ui/UpgradePrompt';
import { Skeleton } from '@/components/ui/Skeleton';
import { InsightCard } from '@/features/ai/components/InsightCard';
import { LoadingSpinnerFullPage } from '@/components/ui/LoadingSpinner';
...
const Reports: React.FC<ReportsProps> = ({ preselectedTeamId, preselectedPlayerId, fromSessionDetails, onTeamChange }) => {
  ...
  const handleGenerateInsights = async () => {
    ...
    const { allowed, used, limit, remaining } = await subscriptionService.canUseAIInsights();
    
    if (!allowed) {
      alert(`Você atingiu o limite de ${limit} insights de IA este mês. Upgrade para Premium para insights ilimitados!`);
      return;
    }
    ...
    try {
      ...
      await subscriptionService.incrementAIInsightsUsage();
      
      // Reload subscription to update quota display
...
Gerado por BaseCoach - Plataforma de Análise de Desempenho para Futsal`;
  ...
  if (loading) {
    return <LoadingSpinnerFullPage size="lg" label="Carregando relatórios..." />;
  }
    ...
    return (
      <div className="p-6 max-w-4xl mx-auto">
        <EmptyState
          ...
          description="Crie um time em 'Times' para começar a gerar relatórios de desempenho."
          variant="info"
        />
      </div>
    );
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="flex flex-col gap-2">
              <div className="flex gap-2">
                  ...
                  className={`flex items-center gap-2 h-12 px-4 py-3 rounded-lg transition-all duration-200 active:scale-95 ${
      ...
      {/* AI Insights Section */}
      {subscription && TIER_FEATURES[subscription.tier].aiInsights ? (
        <div className="bg-gradient-to-br from-indigo-50 to-purple-50 rounded-xl shadow-sm border-2 border-indigo-200 p-6">
          <div className="flex items-center justify-between mb-4">
            <div className="flex-1">
              ...
              {/* Quota Display for Pro users */}
              {subscription.tier === 'pro' && (
                <p className="text-xs text-slate-600 mt-1">
                  {subscription.aiInsightsUsed} de {subscription.aiInsightsLimit} insights usados este mês
                  {subscription.aiInsightsUsed >= subscription.aiInsightsLimit && (
                    <span className="text-orange-600 font-medium"> • Limite atingido - Upgrade para Premium para insights ilimitados</span>
                  )}
                </p>
              )}
          ...
          {loadingInsights ? (
            <div className="space-y-4">
              ...
              <div className="text-center text-sm text-slate-500 mt-4">
                Analisando dados do atleta com IA...
              </div>
            </div>
          ) : aiInsights ? (
            <div className="space-y-6">
              ...
              {/* Overall Assessment */}
              <div className="bg-gradient-to-br from-indigo-50 to-purple-50 rounded-xl p-6 border-2 border-indigo-200 shadow-sm">
                <h4 className="font-bold text-indigo-900 mb-3 text-lg tracking-tight">Avaliação Geral</h4>
        ...
        <UpgradePrompt
          ...
          description="Receba análises inteligentes e personalizadas sobre o desempenho dos atletas, com pontos fortes, áreas de melhoria e recomendações de treino geradas por inteligência artificial."
          requiredTier="pro"
          size="medium"
          ctaText="Começar com 5 Insights/Mês"
        />
      )}
        </>
      )}
